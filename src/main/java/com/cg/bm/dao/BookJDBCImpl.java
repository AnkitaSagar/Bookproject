...
import com.cg.bm.util.ConnectionProvider;
...
public class BookJDBCImpl implements IBook{
	...
	public BookJDBCImpl() throws BookStoreException {
	...
	public boolean delete(String bcode) throws BookStoreException {
		...
		try (Connection con = conProvider.getConnection();
				PreparedStatement pDelete = con
						.prepareStatement(IQueryMapper.DEL_BOOK)) {
	...
	public Book get(String bcode) throws BookStoreException {
		...
		try (Connection con = conProvider.getConnection();
				PreparedStatement pSelect = con
						.prepareStatement(IQueryMapper.GET_BOOK)) {
	...
	public List<Book> getAll() throws BookStoreException {
		...
		try (Connection con = conProvider.getConnection();
				PreparedStatement pSelect = con
						.prepareStatement(IQueryMapper.GET_ALL_BOOKS)) {
		...
		if (book != null) {
			try (Connection con = conProvider.getConnection();
					PreparedStatement pUpdate = con
							.prepareStatement(IQueryMapper.MODIFY_BOOK)) {
